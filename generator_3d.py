...
def main():
    ...
    from remap_fields_2d import effective_mapped_radius
    _user_r = 0.0
    try:
        _hp = os.path.join(SOURCE_1D_CASE, "ggui_remap_handoff.json")
        if os.path.isfile(_hp):
            with open(_hp, encoding="utf-8") as _hf:
                _user_r = float((json.load(_hf).get("remap_radius_m") or 0.0))
    except Exception:
        _user_r = 0.0
    R_remap = effective_mapped_radius(r_1d, _user_r)
    mask = R <= R_remap
